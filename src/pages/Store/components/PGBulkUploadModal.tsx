...
import { usePreviewPGBulkUpload, useExecutePGBulkUpload, useToast } from '@/hooks';
import { parsePGBulkUploadExcel, downloadPGBulkUploadTemplate } from '@/utils/excel';
import { validateExcelFile } from '@/utils/fileValidation';
import type { PGBulkUploadRow, BulkUploadPreviewItem } from '@/types/store';

interface PGBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export const PGBulkUploadModal: React.FC<PGBulkUploadModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const [parsedRows, setParsedRows] = useState<PGBulkUploadRow[]>([]);
  ...
  const previewMutation = usePreviewPGBulkUpload();
  const executeMutation = useExecutePGBulkUpload();
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows = await parsePGBulkUploadExcel(file);
  ...
  const handleExecute = async () => {
    try {
      ...
      if (result.success > 0 && result.failed === 0) {
        toast.success(`${result.success}건의 PG 정보가 업데이트되었습니다.`);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="PG 일괄 업로드" size="lg">
          ...
          <>
            ...
            <div className="flex items-center justify-between p-4 bg-bg-secondary rounded-lg">
              <div>
                ...
                <p className="text-xs text-txt-muted mt-1">
                  매장명, 사업자번호, PG사, MID, API키 컬럼이 필요합니다.
                </p>
              </div>
              <Button variant="outline" size="sm" onClick={downloadPGBulkUploadTemplate}>
            ...
            <div className="text-sm text-txt-muted">
              ...
              <ul className="list-disc pl-5 space-y-1">
                ...
                <li>PG사: smartro, kcp, nicepay, toss, other</li>
                <li>API키는 선택사항입니다.</li>
              </ul>
            </div>
          </>
                ...
                <tbody>
                  {previewItems.map((item) => (
                    ...
                    >
                      ...
                      <td>{item.data.pgVendor}</td>
                      <td className="font-mono text-sm">{item.data.mid}</td>
