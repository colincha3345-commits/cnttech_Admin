...
export const productToCsvRow = (product: Product): string[] => {
  return [
    ...
    product.allowOwnDiscount ? 'Y' : 'N',
    product.allowPartnerDiscount ? 'Y' : 'N',
    product.productCode || '',
    product.posCode || '',
...
export const csvRowToProductFormData = (row: string[]): ProductFormData => {
  ...
  return {
    ...
    mainCategoryId,
    subCategoryIds: row[5] ? row[5].split('|').filter(Boolean) : [],
    optionGroupIds: [],
    ...
    allowOwnDiscount: row[10] !== 'N',
    allowPartnerDiscount: row[11] !== 'N',
