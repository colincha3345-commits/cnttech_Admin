...
export const csvRowToProductFormData = (row: string[]): ProductFormData => {
  ...
  return {
    ...
    posCode: row[12] || undefined,
    origin: [],
    nutrition: {
      calories: parseInt(row[13] || '0', 10),
      sodium: parseInt(row[14] || '0', 10),
      carbs: parseInt(row[15] || '0', 10),
      sugar: parseInt(row[16] || '0', 10),
      fat: parseInt(row[17] || '0', 10),
      protein: parseInt(row[18] || '0', 10),
      servingSize: row[19] || '',
    },
    allergens: row[20] ? row[20].split('|').filter(Boolean) : [],
    badgeIds: row[22] ? row[22].split('|').filter(Boolean) : [],
    displayOrder: parseInt(row[21] || '999', 10),
  };
};
