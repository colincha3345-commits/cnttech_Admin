...
| 3 | "산출완료" 행 클릭 | 상세 페이지 이동 | 정산 구성요소 표 렌더링 |
| 4 | 매출/배달비/할인/수수료/실정산액 확인 | 각 항목 천단위 콤마 + ₩ 표기 | netAmount 볼드 강조 |
| 5 | 하단 주문별 상세 테이블 확인 | 개별 주문 목록 렌더링 | 주문번호, 금액, 결제수단 |
| 6 | [지급완료] 클릭 | ConfirmDialog → `PATCH /api/settlements/:id/status` | status→completed, paymentDate 기록 |
...
```
netAmount = totalSales + deliveryFee + hqSupport - promotionDiscount - platformFee

각 항목 상세:
  totalSales: 기간 내 completed 주문의 상품 합계
  deliveryFee: 배달 주문 배달비 합계
  hqSupport: 본사 부담 할인금 (할인/쿠폰의 headquartersRatio 비율분)
  promotionDiscount: 총 할인액 (가맹점 부담분 포함)
  platformFee: 플랫폼 수수료 (매출 × 수수료율)

주의: netAmount는 음수 가능 (수수료 > 매출 시)
```

### 4.2. 정산 상태 전이 규칙

```
pending → calculated : 배치 산출 완료 시 자동 전환
calculated → completed : 관리자가 지급 확인 후 수동 전환
(어느 단계든) → on_hold : 보류 처리 (사유 필수 입력)
on_hold → calculated : 보류 해제 시 (관리자 수동)
```

### 4.3. 정산 스냅샷 정책

```
배치 실행 시점의 주문 데이터 기준으로 산출
배치 실행 중 발생하는 주문 변경(취소 등)은 다음 정산에 반영
스냅샷 시점: 정산 기간 종료일 23:59:59 기준
```

### 4.4. 본사 지원금(hqSupport) 계산

```
프로모션별: discountAmount × (headquartersRatio / 100)
가맹점 수입 합산: hqSupport는 가맹점이 받을 금액에 더함
