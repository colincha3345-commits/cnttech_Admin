...
- 2026-03-23: 영업정보 공통 스타일 통일 및 기능 추가 — (1) 간편 설정(평일/주말/공휴일)에 누락된 휴게시간 추가, (2) 배달/포장 시간 Input w-32→w-36 h-12 확대, (3) time/date input 전체 영역 클릭 시 picker 열리도록 CSS 추가(globals.css), (4) 임시휴업 사유 텍스트→버튼 선택(재료소진/기기점검/개인사정/기타)으로 변경+기타 시 직접입력, (5) 날짜 Input h-12 확대, (6) label 스타일 통일(text-sm font-medium text-txt-secondary mb-2).
