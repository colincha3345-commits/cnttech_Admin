...
1. **계정 목록 조회** — 좌측 패널에 관리자 계정 목록을 표시한다. 검색, 상태별(활성/비활성) 필터를 제공한다.
2. **권한 설정** — 우측 패널에 선택된 계정의 메뉴별 접근 권한을 설정한다. 메뉴 그룹 단위 일괄 설정 및 개별 토글을 지원한다.
3. **저장** — 변경사항 감지 시 저장/초기화 버튼 활성화. 저장 시 감사 로그를 기록한다.
4. **제약** — 권한관리(permissions) 메뉴의 쓰기(write) 권한이 있는 계정만 다른 계정의 권한을 수정할 수 있다.
...
| **계정 카드** | Card | - | - | 이름, 이메일, 부서, 상태 표시다. |

#### 권한 설정 (메뉴별)

각 메뉴(1뎁스)마다 하위 권한(2뎁스)을 체크박스로 설정한다. 메뉴에 따라 사용 가능한 권한 항목이 다르다.

| 권한 항목 | 입력 형태 | 설명 |
| :--- | :---: | :--- |
| **조회 (view)** | Checkbox | 해당 메뉴에 접근하여 목록/상세를 열람할 수 있다. 해제 시 메뉴 자체가 비노출된다. |
| **쓰기 (write)** | Checkbox | 데이터를 생성/수정/삭제할 수 있다. 조회가 꺼지면 자동으로 비활성된다. |
| **마스킹 해제 (masking)** | Checkbox | 개인정보(이름, 연락처 등)의 원문을 조회할 수 있다. 해제 시 `홍**` 형태로 표시된다. |
| **다운로드 (download)** | Checkbox | 목록 데이터를 엑셀 등으로 내보내기할 수 있다. |

**메뉴별 권한 트리 구조 (1뎁스: 메뉴 / 2뎁스: 하위 권한)**
...
📂 권한 관리 (permissions)
 ├─ ☐ 권한 조회
 └─ ☐ 권한 설정 (쓰기) ← 이 권한이 있어야 다른 계정의 권한을 수정할 수 있다
...
> **사용 용도:** 관리자가 직원마다 접근할 수 있는 메뉴와 기능 범위를 다르게 설정하는 데 사용한다. 예를 들어 CS 담당자에게는 고객센터와 주문관리만 허용하고, 정산 담당자에게는 정산관리만 허용하는 식으로 업무에 맞게 권한을 나눌 수 있다.
...
**[API 및 비즈니스 로직 제약사항]**
- **최초 초대 시 기본 권한** — 사용자를 초대하면 모든 메뉴의 권한(조회/쓰기/마스킹 해제/다운로드)이 **사용 가능(true)** 상태로 설정된다. 초대 후 권한관리 담당자가 필요에 따라 권한을 제한할 수 있다.
- **권한 수정 자격** — 권한관리(permissions) 메뉴의 쓰기(write) 권한이 있는 계정만 다른 계정의 권한을 수정할 수 있다.
...
| 3 | 메뉴별 접근 권한 토글 변경 (조회/쓰기/마스킹/다운로드) | 체크박스 변경 | 즉시 상태 반영 |
...
역할(role) 개념 없이, **메뉴별 4가지 권한 토글**로만 접근을 제어한다.

```
view=false  : 해당 메뉴 비노출 (라우트 접근 시 403)
view=true   : 조회 가능 (목록/상세 열람)
write=true  : 생성/수정/삭제 가능 (view 필수 선행)
masking=true: 개인정보 원문 조회 가능
download=true: 데이터 내보내기(엑셀 등) 가능
```
