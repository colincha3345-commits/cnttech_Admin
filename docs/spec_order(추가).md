...
| `/orders/abnormal` | 이상주문 목록 (AbnormalOrderList) | orders:read |
...
6. **이상주문 목록** (`AbnormalOrderList`) — 접수 대기(pending) 상태에서 8분 이상 경과된 주문을 자동 감지하여 별도 목록으로 표시한다. 30초 주기로 자동 새로고침하며, 경과 시간을 실시간 표시한다. 주문유형·매장·키워드 필터를 제공한다. 좌측 네비게이션에 이상주문 건수 Badge를 표시한다.
...
#### 이상주문 목록 (AbnormalOrderList)

| 기능 / 필드명 | 입력/노출 형태 | 필수 여부 | 비고 (UI/UX) |
| :--- | :--- | :---: | :--- |
| **이상주문 기준** | 시스템 자동 | Y | pending 상태 + 8분 이상 경과한 주문을 이상주문으로 판정한다. |
| **경과 시간** | 실시간 텍스트 | Y | "N분 경과" 형식으로 매초 갱신한다. 10분 이상은 빨간색 강조 표시한다. |
| **자동 새로고침** | 폴링 | Y | 30초 주기로 자동 재조회(`refetchInterval: 30000`)한다. |
| **주문유형 필터** | Select | N | 전체/배달/포장/매장식사 선택한다. |
| **매장 필터** | Select | N | 매장 선택 가능하다. |
| **키워드 검색** | Input | N | 주문번호, 회원명, 매장명으로 검색한다. |
| **네비게이션 Badge** | Badge (빨간색) | Y | 좌측 메뉴에 이상주문 건수를 실시간 표시한다(`useAbnormalOrderCount`). |
| **행 클릭** | 주문 상세 이동 | Y | 클릭 시 `/orders/:id` 상세 페이지로 이동한다. |
