...
| :---: | :--- | :--- | :--- |
| 1 | 고객센터 → 1:1 문의 진입 | 미답변 우선 정렬 목록 로드 | status=pending 상단 |
| 2 | 문의 행 클릭 | 상세 페이지: 문의 내용 + 답변 폼 | 고객 정보 표시 |
| 3 | 답변 내용 입력 후 [답변 등록] | `POST /api/inquiries/:id/reply` | status→answered |
| 4 | 고객앱에서 답변 확인 | 알림(푸시) + 문의 상세에서 답변 표시 | 답변 시각 기록 |

### 시나리오 2: FAQ 등록

| 단계 | 사용자 행동 | 시스템 응답 | 검증 포인트 |
| :---: | :--- | :--- | :--- |
| 1 | FAQ 관리 → [FAQ 등록] | 카테고리/질문/답변 입력 폼 | 필수 필드 |
