...
1. **메뉴 목록 조회** — 카테고리, 상태, 키워드 필터를 조합하여 목록을 조회한다. 판매 상태별 Badge(판매중=success, 중지=critical, 대기=warning)를 표출한다.
2. **메뉴 등록/수정** — 상세 페이지에서 기본정보(이름, 가격, 설명, 이미지), 카테고리 설정, 옵션 그룹 연결, 판매 설정, 가맹점 적용, 상세 정보(원산지/영양정보/알레르기)를 입력한다.
3. **일괄 변경** — 목록에서 다중 선택 후 상태/가격/노출 여부를 일괄 변경한다. 변경 결과(성공/실패 건수)를 Toast로 안내한다.
4. **노출 순서 관리** — 드래그 앤 드롭 또는 순서 입력으로 표시 순서를 조정한다.
...
| 기능 / 필드명 | 입력/노출 형태 | 필수 여부 | 글자수 / 제약조건 | 비고 (UI/UX) |
| :--- | :--- | :---: | :--- | :--- |
| **메뉴명 (name)** | Input | Y | 2 ~ 50자 | 앱 노출 타이틀이다. |
| **가격 (price)** | Number Input | Y | 0 이상, 정수 | 천 단위 콤마 포맷을 적용한다. |
| **설명 (description)** | Textarea | N | 최대 500자 | 줄바꿈 허용한다. |
| **대표 이미지** | File Upload | Y | 5MB 이하, jpg/png/webp | 미리보기 썸네일을 제공한다. |
| **서브 이미지** | Multi File Upload | N | 개당 5MB 이하, 최대 5장 | 드래그로 순서 변경 가능하다. |
...
| **노출 여부 (isVisible)** | Switch | Y | Boolean | 비노출 메뉴는 앱에서 숨김 처리한다. |
...
| :--- | :--- | :---: | :--- | :--- |
| **id (PK)** | UUID | Y | 36자 | 시스템 고유 식별자다. |
| **name** | String | Y | 2 ~ 50자 | 앱 검색 인덱싱 대상이다. |
| **price** | Integer | Y | 0 이상 | 음수 불가, 단위: 원(KRW)이다. |
